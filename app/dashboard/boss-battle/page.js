...
export default function BossBattlePage() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await fetch('/api/generate-boss-battle', {
        ...
        body: JSON.stringify({
          gradeLevel, subject, topic, bossTheme, difficulty,
          numRounds, teamBased, includeHealthBar, includePowerUps,
        }),
      })
      ...
      alert('Error generating boss battle. Please try again.')
    }
    ...
    try {
      await fetch('/api/documents', {
        ...
        body: JSON.stringify({
          ...
          content,
          metadata: { gradeLevel, subject, topic, bossTheme, difficulty },
        }),
      })
      setSaved(true)
    } catch (error) {
  ...
  const handleExportDocx = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        a.href = url
        a.download = `Boss_Battle_${topic.replace(/\s+/g, '_')}.docx`
  ...
  const handleCopy = () => {
    navigator.clipboard.writeText(generatedBattle)
    setCopied(true)
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 mb-6">
          <div className="flex items-start justify-between mb-2">
            <div>
              ...
              <p className="text-gray-500">Create epic review games where students defeat the boss with correct answers.</p>
            </div>
          ...
          <div className="mb-5">
            <label className="block text-sm font-medium text-gray-700 mb-2">Topic to Review: *</label>
            <input type="text" value={topic} onChange={(e) => setTopic(e.target.value)} placeholder="e.g., Multiplication Facts, Vocabulary Unit 3, States of Matter"
          ...
          <div className="grid grid-cols-2 gap-4 mb-5">
            <div>
              ...
              <select value={bossTheme} onChange={(e) => setBossTheme(e.target.value)}
                className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-purple-500 text-gray-700 appearance-none cursor-pointer">
                {['Dragon', 'Math Wizard', 'Grammar Goblin', 'Science Monster', 'History Hunter', 'Vocabulary Vampire', 'Robot Overlord', 'Pirate Captain', 'Alien Invader', 'Evil Professor'].map(t => (
            ...
            <div>
              ...
              <select value={difficulty} onChange={(e) => setDifficulty(e.target.value)}
                className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-purple-500 text-gray-700 appearance-none cursor-pointer">
                <option value="easy">Easy (more time, simpler questions)</option>
                <option value="medium">Medium (balanced)</option>
                <option value="hard">Hard (faster pace, harder questions)</option>
          ...
          <div className="mb-6 space-y-3">
            <label className="flex items-center gap-3 cursor-pointer">
              ...
              <div>
                <span className="text-gray-700">Team-based battle</span>
                <p className="text-sm text-gray-500">Students work in teams to defeat the boss together</p>
              </div>
            </label>
            <label className="flex items-center gap-3 cursor-pointer">
              <input type="checkbox" checked={includeHealthBar} onChange={(e) => setIncludeHealthBar(e.target.checked)}
                className="w-5 h-5 text-purple-600 rounded focus:ring-purple-500" />
              <div>
                <span className="text-gray-700">Include health bar tracker</span>
                <p className="text-sm text-gray-500">Visual progress showing boss HP decreasing</p>
              </div>
            </label>
            <label className="flex items-center gap-3 cursor-pointer">
              <input type="checkbox" checked={includePowerUps} onChange={(e) => setIncludePowerUps(e.target.checked)}
                className="w-5 h-5 text-purple-600 rounded focus:ring-purple-500" />
              <div>
                <span className="text-gray-700">Include power-ups</span>
                <p className="text-sm text-gray-500">Special bonuses like double damage or skip questions</p>
              </div>
            </label>
          </div>
        ...
        <div ref={outputRef} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
          ...
          {generatedBattle ? (
            <div className="bg-gray-50 rounded-xl p-5 min-h-[200px] max-h-[500px] overflow-y-auto">
              <pre className="whitespace-pre-wrap text-gray-700 text-sm font-sans leading-relaxed">{generatedBattle}</pre>
            </div>
          ) : (
            <div className="bg-gray-50 rounded-xl p-5 min-h-[200px] flex items-center justify-center">
              <div className="text-center">
                <div className="text-4xl mb-3">🐉</div>
                <p className="text-gray-400">Your generated boss battle will appear here</p>
              </div>
            </div>
          )}
        </div>
